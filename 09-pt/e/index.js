...
function clicar(e) {
    ...
    if (numberOne.value && numberTwo.value) {
        ...
        try {
            if (num2 === 0)
                throw `Impossible to divide if divisor = 0`;
            if (!Number.isInteger(num1) || !Number.isInteger(num2) || num1 < 0 || num2 < 0)
                throw `Impossible to divide ${num1} / ${num2}, use integers and positive numbers`;
        } catch (error) {
            numberOne.value = '';
            numberTwo.value = '';
...
function divide(num1, num2) {
    ...
    return document.querySelector('h1').innerHTML = `${somar(1, divide(subtract(num1, num2), num2))}, com resto ${num1 % num2}`;
